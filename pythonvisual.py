...
def build_plot_values(gdpinfo, gdpdata):
    ...
    """
    gdplist = []
    yearlist = list(range(gdpinfo["min_year"], gdpinfo["max_year"]+1))
    for year in yearlist:
        if str(year) in gdpdata and gdpdata[str(year)] != '':
            gdp = float(gdpdata[str(year)])
            tup = (year, gdp)
            gdplist.append(tup)
    return gdplist
...
def build_plot_dict(gdpinfo, country_list):
    ...
    """
    plot_dict = {}
    filedict = read_csv_as_nested_dict(gdpinfo["gdpfile"], gdpinfo["country_name"],
                                       gdpinfo["separator"], gdpinfo["quote"])
    for country in country_list:
        if country in filedict:
            gdpdata = filedict[country]
...
def render_xy_plot(gdpinfo, country_list, plot_file):
    ...
    xy_plot.y_title = "GDP in currents US dollars"
    for country in country_list:
        xy_plot.add(country, plot_dict[country])
